...
class Checkers:
    ...
    def minmax(self, player, depth=1, RL=False, verbose=False):
        Leafs = []
        FinalMoves = []
        b1 = self.CopyBoard()
        s1 = self.score1
        ...
        for m1 in moves:
            ...
            if depth == 1:
                ...
                # Depth > 1: simulate opponent reply
                b2 = self.CopyBoard()
                s21 = self.score1
                s22 = self.score2
                opponent_moves = self.GetValidMoves(-player, Filter=True)
                if len(opponent_moves) == 0:
                    Leafs.append((m1, 100, self.GetFeatures(player)))
                for m2 in opponent_moves:
                    self.SetBoard(b2)
                    self.score1 = s21
                    self.score2 = s22
                    self.PushMove(m2)
                    # Now call minmax again at one less depth
                    future_leafs = self.minmax(player, depth=depth-1, RL=True)
                    if future_leafs:
                        score = future_leafs[0][1]  # Get first move's score
                    else:
                        score = self.GetScore(verbose=False, player=player)   # Get first move's score
                    Leafs.append((m1, score, self.GetFeatures(player)))
        ...
        for leaf in Leafs:
            if leaf[1] > maximum:
                maximum = leaf[1]
                FinalMoves.clear()
                if not self.CheckIfInside(leaf[0][0], leaf[0][1], FinalMoves):
    ...
    def playRandomMinMax(self, Bot1="random", Bot2="random", verbose=False, GetFeatures=False, ArrayFeatures=[]):
        ...
        while True :
            ...
            if (player == 1):
                ...
                elif Bot2 == "curriculum":
                    # Use a moving winrate estimate (you can improve this with history later)
                    if not hasattr(self, "curriculum_randomness"):
                        self.curriculum_randomness = 0.8  # Start with 80% random

                    # Slowly decrease randomness if player 1 wins often (simulate curriculum)
                    if hasattr(self, "last_result"):
                        if self.last_result == -1:  # player 2 won last time
                            self.curriculum_randomness = max(0.1, self.curriculum_randomness - 0.1)

                    if random.random() < self.curriculum_randomness:
                        move = random.choice(moves)
                    else:
                        move = random.choice(self.minmax(-1))
...
if __name__ == "__main__":
    # Comment this out when training, only run if manually testing
    # main()
    pass
